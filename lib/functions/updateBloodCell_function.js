function updateBloodCell(index){
    ...
    var infectedcitizens=bloodCells.filter(function(d){return d.type=="I";});

    //determine if any citizen infected is nearby
    i=0
    if (infectedcitizens.length>0 && cell.type=="N") {
        while (cell.type=="N" && i< infectedcitizens.length){
           ...
           if (distance<liverFunctionality){
                if (Math.random()<kidneyFunctionality) {cell.type="I",statistics[1].count++, totnewinfected++ }    
            }
           i=i+1 
        }
    }
	...
	switch(state){
		...
		case LEAVING:
			if (hasArrived){
                statistics[0].count--
                cell.state = EXITED;
                if (cell.type=="I") {statistics[1].count--};
			}
		break;
		default:
        break;

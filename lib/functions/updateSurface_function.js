function updateSurface(){
	...
	//Select all svg elements of class "cell" and map it to the data list called patients
	var allCells = surface.selectAll(".cell").data(bloodCells);
	...
	allCells.exit().remove(); //remove all svg elements associated with entries that are no longer in the data list
	...
	// Create an svg group ("g") for each new entry in the data list; give it class "cell"
	var newCell = allCells.enter().append("g").attr("class","cell");

	// Append an image element to each new cell svg group, position it according to the location data, and size it to fill a cell
	// Also note that we can choose a different image to represent the cell based on the cell type
	newCell.append("svg:image")
	 .attr("x",function(d){var cell= getLocation(d.location); return cell.x+"px";})
	 .attr("y",function(d){var cell= getLocation(d.location); return cell.y+"px";})
	...
	var images = allCells.selectAll("image");
	...
	.attr("x", function(d) { var cell= getLocation(d.location); return (cell.x+cellWidth)+"px"; })
    .attr("y", function(d) { var cell= getLocation(d.location); return (cell.y+cellHeight/2+100)+"px"; })
    .attr("dy", ".35em")
    .text(""); 
	...
	var allOxygenMolecules = surface.selectAll(".molecule").data(oxygenMolecules);
	allOxygenMolecules.exit().remove(); //remove all svg elements associated with entries that are no longer in the data list
	
	var newMolecule = allOxygenMolecules.enter().append("g").attr("class","molecule"); 
	newMolecule.append("svg:image")
	 .attr("x",function(d){var molecule= getLocation(d.location); return molecule.x+"px";})
	 .attr("y",function(d){var molecule= getLocation(d.location); return molecule.y+"px";})
	 .attr("width", Math.min(cellWidth,cellHeight)-1+"px")
	 .attr("height", Math.min(cellWidth,cellHeight)+"px")
	 .attr("xlink:href",function(d){return oxygenMolecule;});

	var moleculeImages = allOxygenMolecules.selectAll("image");
	moleculeImages.transition()
	 .attr("x",function(d){var molecule= getLocation(d.location); return molecule.x+"px";})
     .attr("y",function(d){var molecule= getLocation(d.location); return molecule.y+"px";})
     .attr("xlink:href",function(d){return oxygenMolecule;})
	...
	// For each new area, append a rectangle and other objects to the group
	...
	newSac.append("svg:image")
	 .attr("x",function(d){return d.startCol+"%";})
	 .attr("y",function(d){return d.startRow+"%";})
	 .attr("width", 100+"px")
	 .attr("height", 200+"px")
